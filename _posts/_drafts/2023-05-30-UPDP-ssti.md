...
if __name__ == '__main__':
    app.run()
```
Lo que esta haciendo es que cuando se visite la ruta ```/```, va a renderizar la plantilla con el nombre ```plantilla.html``` con los valores proporcionados, en este caso, el nombre, que tiene como valor ```GuxFiz```, para este ejemplo se esta usando ```render_template``` el cual renderiza desde un acrhivo, pero tambien puede ser usado ```render_template_string```, que renderiza desde un string, de esta forma:
